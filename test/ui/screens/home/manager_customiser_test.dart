...
void main() {
  ...
  group('A LOCKED CHIP THAT A VIDEO CAN OPEN', () {
    ...
    testWidgets('AND PICKING SOMETHING ELSE TAKES THE BAR AWAY', (
      tester,
    ) async {
      // The ✕ is a way out, not the only one: the bar sits over the picture,
      // and a player who has moved on to another chip has answered it. Two of
      // the three routes `_clearOffer` documents actually did this — the ✕ and
      // changing axis — and tapping another chip left the bar standing over a
      // figure that had already changed. Reported as having to press the ✕.
      phone(tester);
      await pumpHome(tester);
      await openCustomiser(tester);
      await openAxis(tester, 'beard');
      ...
      // 'none' is the one every save starts with, so it is always unlocked.
