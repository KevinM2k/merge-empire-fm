...
void main() {
  ...
  group('boosts and consumables', () {
    ...
    testWidgets('the three coin consumables are shown but not yet buyable', (
      tester,
    ) async {
      // Their purchase logic lives inside the JS screen rather than an engine,
      // so there is nothing ported to call. Shown priced rather than hidden.
      await pumpShopWidget(tester, (_) {}, BoostsSection.new);
      for (final row in coinConsumables) {
        expect(
          tester
              .widget<ElevatedButton>(
                find.byKey(ValueKey('shop-buy-coin-${row.id}')),
              )
              .onPressed,
          isNull,
          reason: row.id,
        );
        expect(find.text(t(row.nameKey)), findsOneWidget, reason: row.id);
      }
    });
  });
