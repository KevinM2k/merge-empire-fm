...
const Map<String, String> zhCopy = <String, String>{
  ...
  // ── 点球大战，一个一个地播 ────────────────────────────────────────
  //
  // JS 里这段演出是写死的英文，没有 `t()` 键，生成目录里没有可移植的东西。
  // 这是沙发上直接提的需求，所以在这里用十种语言写出来。{score} 由行来画，
  // 不由句子来画。
  'match.pens.head': '点球大战',
  'match.pens.going':
      '要踢点球了。|'
      '九十分钟难分高下，胜负交给十二码。|'
      '仍然平手，晋级名额在点球点上决定。',
  ...
  'match.pens.opp_missed':
      '扑出来了！门将判断正确，{them}罚丢了。|'
      '{them}打偏了，擦柱而出。|'
      '{them}打高了，天大的失误。',
  'match.pens.through': '{us}赢下点球大战，晋级。',
  'match.pens.out': '{them}赢下点球大战，{us}出局。',

  // ── 以及一场打到点球大战的杯赛 ────────────────────────────────────
  //
  // 新增的键：生成目录所依据的 JS 战报里根本没有点球大战。它们取代平局的
  // 标题。{score} 是九十分钟的比分，{pens} 是点球大战的比分，都以主队在前。
  'report.cup.pens_won':
      '全场结束，还是要靠点球：九十分钟{score}，{club}以{pens}赢下点球大战，'
          '晋级。|'
      '哨声响起时是{score}。{club}在十二码前稳住了：{pens}，{opp}出局。|'
      '结束，{score}，点球大战定胜负——{pens}，{club}晋级。',
  'report.cup.pens_lost':
      '全场结束，还是要靠点球：九十分钟{score}，{club}以{pens}告负，出局。|'
      '哨声响起时是{score}。{club}没能在十二码前了结：{pens}，{opp}晋级。|'
      '结束，{score}，点球大战定胜负——{pens}，{club}的杯赛之路到此为止。',
  ...
  'matchtrait.when.home': "主场",
  'matchtrait.when.away': "客场",
  'matchtrait.when.strongerOpponent': "面对更强对手",
  'matchtrait.when.firstTwenty': "开场二十分钟",
  'matchtrait.when.relegationZone': "身处降级区",
  'matchtrait.when.lastFifteen': "最后十五分钟",
  'matchtrait.when.cupTie': "杯赛",
  'matchtrait.when.superSub': "替补晚登场",
  'matchtrait.when.derby': "德比战",
  'matchtrait.when.tenMen': "少一人作战",
  'matchtrait.when.booked': "吃到黄牌后",
  'matchtrait.when.injuryShrug': "受伤时",
  'matchtrait.when': "触发条件",
  'matchtrait.effect.rating': "评分+{n}%",
  ...
  'boost.park_the_bus.desc': "全员回防——二十五分钟内比赛陷入死寂",
  'boost.var_review.name': "VAR回看",
  'boost.var_review.desc': "撤销一张红牌——球员留在场上，改判黄牌",
  ...
  'boost.bus.over': "枷锁解开，空间重新出现。|阵型松动，两队又能踢起来了。",
  'boost.var.overturned': "VAR推翻了判罚。红牌撤销，{player}留在场上。|主裁走向监视器，回来时摇着头。{player}获得赦免。",
  ...
  'boost.sharp_shooting.name': "精准射术",
  'boost.sharp_shooting.desc': "每个机会都是好机会——25分钟内我方射术更加锋利",
  'boost.quiet_word.name': "悄悄话",
  'boost.quiet_word.desc': "在裁判耳边说句话——抹去一张黄牌",
  'boost.quiet_word.idle': "没有人吃到黄牌",
  'boost.sharp.live': "射门教练发话了。每一脚射门看起来都要进。|鞋带系紧了。来的机会都被打进了。",
  'boost.sharp.over': "热度退去，机会又只是机会。|高光期结束。回到靠自己争取的时候。",
  'boost.quiet.wiped': "跟裁判说了句悄悄话，{player}的黄牌被忘了。|第四官员插了话。{player}的警告进了垃圾桶。",
  'coach.red_card.var_hint': "你有一次VAR回看。趁比赛重开前在替补席申请。",
  'coach.injury.physio_hint': "你有一块神奇海绵。趁比赛重开前在替补席使用。",
  'match.active.title': "生效中",
  'match.active.until': "至第{minute}分钟",
  'matchtrait.name.fortress': "堡垒",
  'matchtrait.desc.fortress': "主场坚不可摧——但仅限主场",
  'matchtrait.name.away_day': "客场英雄",
  'matchtrait.desc.away_day': "享受敌意的球场——客场大放光彩",
  'matchtrait.name.big_game': "大场面球员",
  'matchtrait.desc.big_game': "面对更强对手时挺身而出",
  'matchtrait.name.fast_starter': "闪电开局",
  'matchtrait.desc.fast_starter': "开场即冲——前二十分钟威力惊人",
  'matchtrait.name.relegation_scrapper': "保级斗士",
  'matchtrait.desc.relegation_scrapper': "降级危机越真切，斗志越顽强",
  'matchtrait.name.last_gasp': "最后一口气",
  'matchtrait.desc.last_gasp': "在最后几分钟仍能榨出力量",
  'matchtrait.name.cup_fighter': "杯赛专家",
  'matchtrait.desc.cup_fighter': "为杯赛而生——别处激不出这般斗志",
  'matchtrait.name.super_sub': "超级替补",
  'matchtrait.desc.super_sub': "最后二十分钟替补登场，威力毁灭性",
  'matchtrait.name.derby_devil': "德比恶魔",
  'matchtrait.desc.derby_devil': "为宿敌之战而活",
  'matchtrait.name.ten_man_wall': "十人之墙",
  'matchtrait.desc.ten_man_wall': "凝聚十人——提振场上余下的每一个人",
  'matchtrait.name.ice_veins': "冷血",
