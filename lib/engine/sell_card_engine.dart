...
int _indexOf(Map<String, dynamic>? state, String instanceId) =>
    _cells(state).indexWhere(
      (c) => c is Map<String, dynamic> && c['instanceId'] == instanceId,
    );
...
SaleResult sellCard(
  ...
  if (resources != null) {
    resources['fanCoins'] =
        ((_num(resources['fanCoins']) ?? 0) + coins).toInt();
  }
