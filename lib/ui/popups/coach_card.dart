...
/// **THE SMALL ONE, bottom left — an aside rather than an interruption.**
///
/// [showCoachCard] is a full dialog with a portrait and a row of buttons, and
/// it is the right shape for a decision: a red card, a transfer bid, a sponsor.
/// It is the wrong shape for a remark. A caution while the match is running is
/// a remark — "he's on a yellow, and there's someone better warmed up" — and
/// stopping the game with a card the size of the screen to say it was reported
/// from the couch as too much.
///
/// So this is the home screen's own bubble, off its own corner instead of off
/// the coach dock: `CoachSpeechBubble` with the tail under it, a tap anywhere
/// outside to dismiss, and at most one action. It resolves `true` when the
/// action was taken and null when it was waved away, so a caller can act on it
/// the way it acts on a card's answer.
Future<bool?> showCoachAside(
  BuildContext context, {
  ...
  // The same reasoning as the home bubble: the offsets below are measured off
  // the screen's own corner, and a route that insets its child would push the
  // bubble up by the height of the home indicator.
  useSafeArea: false,
  builder: (context) => _CoachAside(
    body: body,
    label: label,
...
class _CoachAside extends StatelessWidget {
  const _CoachAside({
    required this.body,
    required this.label,
    this.actionLabel,
  ...
  Widget build(BuildContext context) {
    ...
    const left = 14.0;
    final maxWidth = math.max(180.0, math.min(340.0, screen.width - left - 20));
    final action = actionLabel;

    return Stack(
      children: [
        // Anywhere outside closes it, which is the whole difference between an
        // aside and a question.
        Positioned.fill(
          child: GestureDetector(
            ...
            children: [
              Material(
                ...
                child: CoachSpeechBubble(
                  key: const ValueKey('coach-aside'),
                  maxWidth: maxWidth,
                  label: Text(
                    label.toUpperCase(),
                    style: TextStyle(
                  ...
                  closeKey: const ValueKey('coach-aside-close'),
                  onClose: () => Navigator.of(context).pop(),
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(body, style: coachBubbleTextStyle(context)),
                      if (action != null) ...[
                        const SizedBox(height: 8),
                        TextButton(
                          key: const ValueKey('coach-aside-action'),
                          style: TextButton.styleFrom(
              ...
              // The tail, pointing down and back at the corner he stands in.
              Positioned(
                left: 12,
                bottom: -10,
                child: CustomPaint(
